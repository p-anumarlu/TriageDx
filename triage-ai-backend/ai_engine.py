...
─────────────────────────────────────────
Secondary layer in the two-layer blend.
Uses gemini-1.5-flash (free tier) to provide probabilistic assessment
that complements the rule-based safety floor.
...
class GeminiTriageEngine:
    ...
    Fails safely — if the API call fails for any reason, returns a
    conservative fallback result so the rule engine remains the safety floor.
    """
        ...
        if _GENAI_AVAILABLE and settings.GOOGLE_AI_API_KEY:
            ...
            self._model = genai.GenerativeModel(
                ...
                generation_config=genai.GenerationConfig(
                    temperature=0.1,       # Low temp → consistent, conservative outputs
    ...
    async def evaluate(
        ...
        try:
            ...
            return AIEngineResult(
                level=LEVEL_911,   # Fail safe → most conservative
                confidence=0.50,
                red_flag=False,
                conditions=[],
    ...
    def _build_prompt(
        ...
        if extra_context and extra_context.strip():
            prompt += f"\nAdditional patient context: {extra_context.strip()}\n"
    ...
    def _parse_response(raw: str) -> AIEngineResult:
        ...
        level = max(LEVEL_HOME, min(LEVEL_911, level))  # clamp 0–4
    ...
    def _no_api_key_fallback() -> AIEngineResult:
        return AIEngineResult(
            level=2,        # Default to Doctor/Urgent Care — conservative but not alarmist
            confidence=0.40,
            red_flag=False,
